...
defineEmits<{ remove: [id: string]; rename: [c: Class]; view: [c: Class] }>()
  ...
  <div class="st-card !p-0 overflow-hidden">
    <DataTable :value="classes" responsive-layout="scroll" striped-rows paginator :rows="10">
      ...
      <Column header="Students">
        <template #body="{ data }">
          <button type="button" class="font-semibold text-accent hover:underline" @click="$emit('view', data)">
            {{ db.studentsForClass(data.id).length }} <i class="pi pi-external-link text-[10px]" />
          </button>
        </template>
      </Column>
      ...
      <Column header="Actions" :style="{ width: '120px' }">
        <template #body="{ data }">
          <div class="flex gap-1.5">
            <Button icon="pi pi-users"  severity="secondary" outlined size="small" aria-label="View students" @click="$emit('view', data)" />
